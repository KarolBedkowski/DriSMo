...
public class ViewExtraInfo extends BaseActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (speedUnitPref.equals("km/h"))
			speedUnit = getString(R.string.km_h);
		else if (speedUnitPref.equals("mph"))
			speedUnit = getString(R.string.mph);
		else
			speedUnit = getString(R.string.m_s);
		...
		if (size > 0) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (score[i] > Quality.EXCELLENT_SCORE)
					excellentCount++;
				else if (score[i] <= Quality.EXCELLENT_SCORE
						&& score[i] > Quality.GOOD_SCORE)
					goodCount++;
				else if (score[i] <= Quality.GOOD_SCORE
						&& score[i] > Quality.OK_SCORE)
					okCount++;
				else
					badCount++;
			...
			avgBuff /= size;
			avgSpeed.setText((size - nulls > 0) ? ((int) ((speedBuff / (size - nulls)) * Config
					.getSpeedConv())) + " " + speedUnit
					: "--");
			maxSpeed.setText((maxSpeedBuff > 0) ? ((int) (maxSpeedBuff * Config
					.getSpeedConv())) + " " + speedUnit : "--");
			...
			avgScoreText
					.setText(getString(Quality.getQualityStringId(avgBuff)));
			...
			if (Config.getSpeedUnit().equals("mph")) {
				...
				tripLengthString = (new DecimalFormat("#.###"))
						.format(lengthTemp) + " miles";
			} else if (tripLength >= 1000) {
				tripLengthString = (new DecimalFormat("#.###"))
						.format(tripLength) + " km";
			} else
				tripLengthString = tripLength + " m";

			if (!avgSpeed.getText().equals("--")) {
				distanceText.setText(tripLengthString);
				fromLocation.setText(extras.getString("fromStreet") + ", "
						+ extras.getString("fromCity"));
				toLocation.setText(extras.getString("toStreet") + ", "
						+ extras.getString("toCity"));
			...
			totDur.setText(((hour > 0) ? hour + "h " : "")
					+ ((minutes > 0) ? minutes + "m " : "") + totSec + "s");
		} else
			Toast.makeText(getApplicationContext(), R.string.noInfo,
